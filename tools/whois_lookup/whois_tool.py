...
class WhoisInput(BaseModel):
    ...
    timeout: Optional[int] = Field(30, description="Timeout in seconds for the WHOIS query")
...
class WhoisTool(BaseTool):
    ...
    description: str = "Lookup WHOIS information for a domain to retrieve registration details"
    ...
    def _run(self, domain: str, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            if w.domain_name is None:
                return {"error": f"Domain not found or no WHOIS record available for {domain}"}
            ...
            if "No match for domain" in str(e):
                ...
                return {"error": "WHOIS lookup rate limit exceeded. Please try again later."}
            else:
